...
Filename: 190606_AllDataNoPulse_100g500i10m50c
Directory: /pine/scr/a/e/aeallen/Longleaf/190606_AllDataNoPulse_100g500i10m50c
...
###########################################################################
#filename = '/Users/AmyAllen/Documents/ThesisWork/ModelSelection/Mating-Model/Data/Experimental Data/experimental.pickled' #local
filename = '/pine/scr/a/e/aeallen/Longleaf/experimental.pickled' #longleaf
...
def make_conversion_matrix():
    ...
    minimums = [-6.9,-10.3,-4.5,-5,-5,-3.6,-7.1,-10.5,-4.5,-3.7,-12,-2.3,-11,-2.8,-11,-12,-6.1,-9.5,-4.5,-5,-11,-2.7,-3.7,-7.1,-10.5,-4.5,-2.6,-3.7]
    maximums = [-1.6,-1.7,0,-1,-1,-1.5,-1.6,-1.7,0,-1.5,6.8,0,4,1,4,6.8,-0.4,-0.7,0,-1,0,-0.5,-0.5,-0.6,0,0,-1.6,-1.5]
...
arr_synthetic_parameters =[ksynF3,kfb1,KFus3,kp1,kp2,kdegF3,ksynS12,kfb2,KSte12,kdegS12,kff1,km1,ka1,DigsT,ka2,ka3,ksynF1,ka5,KFar1,kp3,kp4,kdegF1,kdegPF1,ksynGFP,ka4,KGFP,slope_on,kdegS12D]
...
def scorefxn1(arr_parameters, time):
   ...
   # signal_numbers = [45,60,75,90,160,200]
     ...
     #   expX = simulate_single_experiment1(arr_params_IP, time, signal_numbers[i],SS)
      #  
       ...
       # idx_closest_time_points = []
        ...
        #SCORE IT! using MSE
        #expX_mse = (np.abs(data_50pulse[i][~np.isnan(data_50pulse[i])] - expX_scorefxn_data[:,0])).mean()
        #print('MSE Exp' + str(idx+1) + ': ', expX_mse)
        #mse_total += expX_mse
    ...
    for i in range(len(signal_numbers)):
        ...
        # get index of time points closest
        idx_closest_time_points = []
        for each_time in times_50periodic[i][~np.isnan(times_50periodic[0])]:
            closest_idx = min(range(len(time)),
                              key = lambda i: abs(time[i] - each_time))
            idx_closest_time_points.append(closest_idx)
        ...
        #SCORE IT! using MSE
        expX_mse = (np.abs(data_50periodic[i][~np.isnan(data_50periodic[0])] - expX_scorefxn_data[:,0])).mean()
        #print('MSE Exp' + str(idx+1) + ': ', expX_mse)
        mse_total += expX_mse
